...
import java.util.Map.Entry;
									/*PRINT THE SUM OF DIAGONAL VALUES OF BINARY TREE*/
...
class GfG
{
	HashMap<Integer,Vector<Integer>> diagonalList = new HashMap<>();
     public void diagonalPrint(TreeNode root)
      {
           int distance = 0;
           diagonalSumtUtil(root,distance);
	  ...
	  public void diagonalSumtUtil(TreeNode root, int distance) {
		  ...
		  if(root.left!=null) {
			  diagonalSumtUtil(root.left,distance-1);
		  }
		  if(root.right!=null) {
			  diagonalSumtUtil(root.right,distance);
		  }
